...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		// Хранилище для данных о поездках
		private ValueState<TaxiRide> rideState;
		// Хранилище для данных о тарифах
		...
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			// Получаем соответствующий тариф из состояния
			TaxiFare fare = fareState.value();

			if (fare != null) {
				// Если тариф найден, создаем пару и очищаем состояния
				out.collect(new Tuple2<>(ride, fare));
				fareState.clear();
			} else {
				// Если тариф еще не пришел, сохраняем поездку
		...
		public void flatMap2(TaxiFare fare, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			// Получаем соответствующую поездку из состояния
			TaxiRide ride = rideState.value();

			if (ride != null) {
				// Если поездка найдена, создаем пару и очищаем состояния
				out.collect(new Tuple2<>(ride, fare));
				rideState.clear();
			} else {
				// Если поездка еще не пришла, сохраняем тариф
